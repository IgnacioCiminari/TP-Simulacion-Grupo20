...
_HEADER_STATS = [
    ...
    "Tiempo_Espera_Auto",
    "Acum_Espera_Autos",
    "Acum_Global_Espera_Autos",
    "Tiempo_Espera_Camioneta",
    "Acum_Espera_Camionetas",
    "Acum_Global_Espera_Camionetas",
    # Bloqueo por línea se agrega dinámicamente (Tiempo_Bloqueo_Li, Acum_Global_Bloqueo_Frenos_Li)
]
...
def _build_header(num_lineas: int) -> list[str]:
    ...
    for i in range(1, num_lineas + 1):
        ...
        header.append(f"Acum_Global_Bloqueo_Frenos_L{i}")
...
class MemoryExporter:
    ...
    def __init__(self) -> None:
        ...
        self._offset_espera_autos: float = 0.0
        self._offset_espera_camionetas: float = 0.0
        self._offset_bloqueo: dict[int, float] = {}
    ...
    def init_header(self, config: "SimulationConfig") -> None:
        ...
        self._offset_espera_autos = 0.0
        self._offset_espera_camionetas = 0.0
        self._offset_bloqueo = {i: 0.0 for i in range(1, self._num_lineas + 1)}
        self.last_row = None
    ...
    def start_day(
        ...
        self._offset_espera_autos = offset_espera_autos
        self._offset_espera_camionetas = offset_espera_camionetas
        if offset_bloqueo:
            self._offset_bloqueo = offset_bloqueo
    ...
    def write_row(
        ...
        acum_global_espera_autos = self._offset_espera_autos + state.total_espera_autos
        acum_global_espera_camionetas = self._offset_espera_camionetas + state.total_espera_camionetas
        ...
        for line in state.lines:
            lid = line.id
            row.extend([
                row_context.get(f"tiempo_bloqueo_l{lid}"),
                self._offset_bloqueo.get(lid, 0.0) + tracker.acum_bloqueo_frenos.get(lid, 0.0),
            ])
